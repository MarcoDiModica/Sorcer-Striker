...
void Enemy::OnCollision(Collider* collider)
{
	...
	if (tipo == Enemy_Type::DOUBLETANK)
	{
		if (cnt == 0) {
			...
			App->particles->AddParticle(App->particles->explosion, collider->rect.x - 25, position.y + 20);
		...
		{
			App->particles->AddParticle(App->particles->BigExplosion, collider->rect.x - 25, position.y + 26);
			App->player->score += 10;
		}
	}
